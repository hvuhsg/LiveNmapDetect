    ...
    if ip.p == dpkt.ip.IP_PROTO_TCP:
        ...
        if len(packetlisttcp) == 0:
            ...
            for i in range(int(len(packetlisttcp))):
                if (
                    eth.ip.src == packetlisttcp[i].ip
                    and eth.ip.dst == packetlisttcp[i].dip
                ):
        ...
        if len(packetlisttcp) > 0 and count == 0:
            for i in range(int(len(packetlisttcp))):
                ...
                if (
                    eth.ip.src != packetlisttcp[i].ip
                    and eth.ip.src != packetlisttcp[i].dip
                ):
                    packetlisttcp.append(packet(eth.ip.src, eth.ip.dst))
                    for i in range(int(len(packetlisttcp))):
                        if (
        ...
        if len(packetlisttcp) > 0 and count1 == 0 and count == 0:
            for i in range(int(len(packetlisttcp))):
                if (
                    eth.ip.src == packetlisttcp[i].dip
                    and eth.ip.dst == packetlisttcp[i].ip
                ):
                    packetlisttcp[i].time.append(ts)
    ...
    if ip.p == dpkt.ip.IP_PROTO_UDP:
        ...
        if len(packetlistudp) == 0:
            ...
            for i in range(int(len(packetlistudp))):
                if (
                    eth.ip.src == packetlistudp[i].ip
                    and eth.ip.dst == packetlistudp[i].dip
                ):
        ...
        if len(packetlistudp) > 0 and count == 0:
            for i in range(int(len(packetlistudp))):
                ...
                if (
                    eth.ip.src != packetlistudp[i].ip
                    and eth.ip.src != packetlistudp[i].dip
                ):
                    packetlistudp.append(packet(eth.ip.src, eth.ip.dst))
                    for i in range(int(len(packetlistudp))):
                        if (
...
for i in range(int(len(packetlisttcp))):
    ...
    for j in range(int(len(packetlisttcp[i].time) - 9)):
        ...
        for x in output1:
            if x not in output:
                output.append(x)
        if (packetlisttcp[i].time[j + 9] - packetlisttcp[i].time[j]) < 0.0005 and len(
            output
        ) > 1:
    ...
    if packetlisttcp[i].st == "B":
        for x in range(int(len(output2))):
            for j in range(int(len(packetlisttcp[i].dport))):
                if (
                    output2[x] == packetlisttcp[i].dport[j]
                    and packetlisttcp[i].dport[j] not in traversed
                ):
                    if len(uportlist) == 0:
                        uportlist.append(j)
...
for i in range(int(len(packetlisttcp))):
    if packetlisttcp[i].st == "B":
        os.system(
            "sudo mergecap -a -w output.pcap myfile.pcap"
        )
...
for i in range(int(len(packetlisttcp))):
    if packetlisttcp[i].st == "B":
        ...
        if len(output1) > 1:
            print(
                "Duration of TCP Scan: ",
                output1[len(output1) - 1] - output1[0],
                "Seconds\n",
            )
...
for i in range(int(len(packetlistudp))):
    ...
    for j in range(int(len(packetlistudp[i].time) - 9)):
        ...
        for x in output1:
            if x not in output:
                output.append(x)
        if (packetlistudp[i].time[j + 9] - packetlistudp[i].time[j]) < 0.0005 and len(
            output
        ) > 1:
